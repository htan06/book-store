...
function section(title, listCard) {
    let html = 
        `<div class="row justify-content-center text-center p-4 bg-light rounded-2 my-2">
            <h2>${title}</h2>
            <div class="row">`

    listCard.forEach(e => {
        html += card(e.img, e.title, e.price)
    });

    html += 
        `</div>
            <button class="btn btn-outline-warning w-25" type="button">Xem thêm</button>
        </div>`
    return html
}
